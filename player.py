...
class Player:
    def __init__(self, owner):
        ...
        self.card = []
        self.card.append(Hero("岩土元素", "中立", 1, [5, 5, 5, 5], [1, 1, 0, 0], ["生物", "元素"], ["坚韧1","损毁"]));
        self.card.append(Hero("骷髅勇士", "中立", 1, [4, 4, 4, 4], [1, 1, 1, 1], ["生物", "骷髅", "战士"], ["损毁"]));
        self.card.append(Hero("精良射手", "中立", 1, [5, 5, 5, 5], [0, 0, 1, 1], ["生物", "人类"], ["箭矢"]));
        self.card.append(Hero("血口黏垢", "中立", 1, [7, 7, 7, 7], [0, 0, 1, 1], ["生物", "流浆"], ["吞食"]));
        self.card.append(Hero("钢指魔像", "中立", 1, [6, 3, 3, 6], [0, 1, 0, 0], ["器械", "魔像"], ["洞穿"]));
        for h in self.card:
            h.owner = owner
        self.pool = []
    ...
    def move(self, bm):
        ...
        self.play_nums += 1
        bm_curx = (ind % bm.total_grid) // bm.height
